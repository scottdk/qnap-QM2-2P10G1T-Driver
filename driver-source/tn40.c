...
static enum PHY_TYPE bdx_phy_init(struct bdx_priv *priv)
{
	...
	/* If no PHY was detected, bdx_mdio_scan_phy_id returns 0. Do not treat
	 * MDIO port 0 as "no PHY" (port 0 is valid).
	 */
	if (phy_id == 0)
	...
	/* Use the discovered MDIO port for PHY reset instead of hardcoded 1 */
