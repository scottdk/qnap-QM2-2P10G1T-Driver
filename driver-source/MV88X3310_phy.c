				...
				unsigned short phy)
{
	...
	/* Verbose diagnostics to help troubleshoot init failures */
	pr_info("MV88X3310: mdio_reset start: priv->phy_mdio_port=%u, param_port=%d, deviceId=0x%04x\n",
		priv->phy_mdio_port, port, priv->deviceId);
	{
		u16 id_hi = bdx_mdio_read(priv, 1, port, 2);
		u16 id_lo = bdx_mdio_read(priv, 1, port, 3);
		pr_info("MV88X3310: mdio port %d PHY ID read: 0x%04x 0x%04x (full 0x%08x)\n", port, id_hi, id_lo, (id_hi << 16) | id_lo);
	}
	...
	do {
		...
		msleep(250);
		{
			u16 c050_before = bdx_mdio_read(priv, 1, port, 0xC050);
			pr_info("MV88X3310: C050 after prep = 0x%04x (expect 0x000a)\n", c050_before);
		}
